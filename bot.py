...
async def get_user_join(id):
    ok = True
    try:
        await codexmania(GetParticipantRequest(channel=channel, participant=id))
        ok = True
...
@codexmania.on(events.ChatAction())
async def _(event):
    if on_join is False:
        return
    if event.user_joined or event.user_added:
        ...
        pp = await codexmania.get_participants(chat)
        count = len(pp)
        ...
        if x is True:
            ...
            butt = [Button.url("Channel", url=f"https://t.me/{channel}"), Button.inline("UnMute Me", data=f"unmute_me{user.id}")]
            await codexmania.edit_permissions(event.chat.id, user.id, until_date=None, send_messages=False)
        
        await event.reply(msg, buttons=butt)


@codexmania.on(events.NewMessage(incoming=True))
async def mute_on_msg(event):
    if event.is_private:
        return
    if on_new_msg is False:
        return
    g = await get_user_join(event.sender_id)
    temp = await codexmania(GetFullUserRequest(event.sender_id))
    if x is False:
        if temp.user.bot:
            return
        nm = temp.user.first_name
        try:
            await codexmania.edit_permissions(event.chat.id, event.sender_id, until_date=None, send_messages=False)
        except Exception as e:
            print(str(e))
            return
...
@codexmania.on(events.callbackquery.CallbackQuery(data=re.compile(b"unmute_me(.*)")))
async def _(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    if uid == event.sender_id:
        g = await get_user_join(uid)
        nm = (await codexmania(GetFullUserRequest(uid))).user.first_name
        if g is False:
            await event.answer(f"You haven't joined @{channel} yet!", cache_time=0, alert=True)
        elif g is True:
            try:
                await codexmania.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
            except Exception as e:
                print(str(e))
                return
